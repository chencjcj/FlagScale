...
def pretrain(
    ...
    if _STARTUP_TIMESTAMPS['program_start'] is not None:
        program_start_global = torch.tensor([_STARTUP_TIMESTAMPS['program_start']], dtype=torch.double, device=cur_platform.device())
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True, config=None, pg_collection=None):
    ...
    if wrap_with_ddp:
        ...
        cur_platform.current_stream().wait_stream(ddp_stream)
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        ##### FlagScale Begin #####
        # NVTX range for step-level profiling annotation.
        if (args.profile
            and (len(args.profile_ranks) == 0 or
                 torch.distributed.get_rank() in args.profile_ranks)
            and iteration >= args.profile_step_start
            and iteration < args.profile_step_end):
            torch.cuda.nvtx.range_push(f"step{iteration}")
        ##### FlagScale End #####
        ...
        if get_num_microbatches() != num_microbatches and iteration != 0 \
            and args.save_when_num_microbatches_change: ####### FlagScale Add #######
        ...
        ##### FlagScale Begin #####
        # NVTX range pop for step-level profiling annotation.
        if (args.profile
            and (len(args.profile_ranks) == 0 or
                 torch.distributed.get_rank() in args.profile_ranks)
            and iteration >= args.profile_step_start
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                ...
                for key in loss_dicts[0].keys():
                    if key not in total_loss_dict:
                        total_loss_dict[key] = torch.tensor([0.0, 0.0], dtype=torch.float, device=cur_platform.device())
